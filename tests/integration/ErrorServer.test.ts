...
describe('ErrorServer', () => {
    ...
    test('labels the 500 block by its heading', () => {
        expect(html).toMatch(/<section class="error[^"]*" aria-labelledby="error-server-title"/);
    });

    test('renders the status code as the glitch heading', () => {
        expect(html).toMatch(/<h1 id="error-server-title" class="error__glitch[^"]*" data-text="500"[^>]*>500<\/h1>/);
