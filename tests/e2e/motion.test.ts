...
test.describe('scroll motion', () => {
    ...
    test('keeps below-fold sections hidden until scrolled into view', async ({ page }) => {
        ...
        const head = page.locator('.experience .section__head');
    ...
    test('shows stagger parents while their children animate in', async ({ page }) => {
        ...
        const cards = page.locator('.highlight');
        ...
        await expect.poll(() => areAllRevealed(page, '.highlight'), POLL).toBe(true);
    });
});
...
test.describe('scroll motion under reduced motion', () => {
    ...
    test('still renders the countdown values', async ({ page }) => {
        await page.goto('/');

        await expect(page.locator('.site-header__countdown-text')).toHaveText(COUNTDOWN_PATTERN);
    });
});
